  ...
  useEffect(() => {
    ...
    const loadPipeline = async () => {
      try {
        ...
        if (!response.ok) {
          if (active) setPipelineError((data as { detail?: string }).detail || "Failed to load lead pipeline.");
          return;
        }
        if (active) {
        ...
        if (active) setPipelineError("Could not connect to lead pipeline API.");
      }
    };
    loadPipeline();
    return () => {
  ...
  return (
    <DashboardShell>
      <h1 style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
        <SovereignIcon icon="compliance_scales" className="w-8 h-8" />
        <span>Admin Governance Control</span>
      </h1>
      <p>Unified governance controls using the shared dashboard card language.</p>
      <TopologySurface>
        <ConstitutionalPanel priority="critical" state={blueprint.admin.controls.dashboards ? "stable" : "degraded"} capability="dashboardControl">
          <TelemetryField label="Dashboard Control" value={blueprint.admin.controls.dashboards ? "Enabled" : "Disabled"} />
        </ConstitutionalPanel>
        <ConstitutionalPanel priority="high" state={blueprint.admin.controls.subscriptionTiers ? "stable" : "watch"} capability="subscriptionTierControl">
          <TelemetryField label="Subscription Tier Control" value={blueprint.admin.controls.subscriptionTiers ? "Enabled" : "Disabled"} />
        </ConstitutionalPanel>
        <ConstitutionalPanel priority="high" state={blueprint.admin.controls.codexUpdates ? "stable" : "watch"} capability="codexUpdateControl">
          <TelemetryField label="Codex Update Control" value={blueprint.admin.controls.codexUpdates ? "Enabled" : "Disabled"} />
        </ConstitutionalPanel>
        <ConstitutionalPanel priority="critical" state={pipelineError ? "degraded" : "stable"} capability="governanceControls">
          <h2>Governance and Deployment Controls</h2>
          ...
          {pipelineError ? <p>{pipelineError}</p> : <p>Pipeline stages: {pipeline.length}</p>}
        </ConstitutionalPanel>
      </TopologySurface>
    </DashboardShell>
  );
